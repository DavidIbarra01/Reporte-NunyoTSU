...
\newcommand{\HoraEvaluacionPesentacion}{10:45 hrs}
...
\newcommand{\AulaEvaluacionOEnlaceMeetPesentacion}{I100-C02} %otras opciones: si es un elance de meet, rodearlo \url{}
...
%\vspace*{1cm}
\section*{Resumen}
La automatización se ha convertido en un privilegio muy demandado, las empresas buscan implementar algoritmos que permitan resolver tareas repetitivas y si la tecnología evoluciona rápido, las necesidades también... y de ahí viene la prioridad de reducir a toda costa los recursos. Es por eso que el proyecto que se planteo durante estas estadías busca como objetivo el poder realizar registros automatizados de los visitantes que van llegando a la recepción del piso 2 de la torre nueva. En vez de andar gastando tinta y papel, los usuarios de la agencia podrán utilizar una plataforma que les permitan tener un mayor control sobre las visitas que van llegando, así mismo podrán tener una mejor administración a la hora de reservar salas de juntas para los visitantes y la mejor parte, es de que todo el proyecto se desarrollo desde un celular, desde la instalación de herramientas clave, hasta programar toda la lógica de negocio del proyecto. El reporte no solo busca demostrar la evidencia que se ha realizado, sino la capacidad de poder crear lo que sea sin importar en que entorno te encuentres, ya que eso es lo que hace un ingeniero... buscar soluciones.
...
%\vspace*{1cm}
\section*{Abstract}
Automation has become a highly sought after asset, companies are looking to implement algorithms to handle repetitive tasks, and as technology evolves rapidly, so do their needs driving a priority to cut resource usage at all costs. Consequently, the project undertaken during this internship aims to automate the registration of visitors arriving at the reception desk on the second floor of the new tower. Instead of wasting ink and paper, agency staff can use a platform that offers greater control over incoming visitors and improves the management of meeting room reservations for guests. Best of all, the entire project was developed on a mobile phone from installing key tools to programming the project's entire business logic. This report aims not only to document the work accomplished but also to demonstrate the ability to create solutions regardless of the environment, because that is what an engineer does... find solutions.
...
\begin{figure}[H]
    ...
    \caption{Mapa de toda la Agencia de Innovación e Inteligencia Digital de Tamaulipas.}
...
Las herramientas que utilizan para desarrollar softwares y plataformas son: Laravel 13,
FastAPI, PostgreSQL para las bases de datos, git con Github, Spatie para el manejo de roles y
privilegios, FusionAuth para validar las credenciales del usuario antes de reidirigirlo al proyecto,
nodejs, composer y Docker para el manejo de contenedores.


\subsection{Ubicación}
Piso 2, Torre Nueva \cite{AIIDT:Ubicacion}.
...
A continuación, la figura \ref{fig:Registro-visitantes} muestra el modulo de visitantes, en donde los usuarios podrán registrar a las visitas que van llegando a la agencia. También la interfaz cuenta con un apartado para la toma de la fotografía del visitante, para así poder agilizar el proceso de búsqueda del visitante con tan solo buscar su rostro mediante embeddings en vez de ir escribiendo los datos del visitante manualmente. Aquí le asignamos el funcionario a visitar, motivo de visita, motivo libre y su gafete.
...
\begin{figure}[H]
    ...
    \caption{UI para la gestion de las llamadas de la plataforma. Diseñado en Figma}
...
Durante esta semana, comencé a pulir las rutas de las API's, como las APIs del registro de visitantes, de visitas, llamadas (las llamadas funcionan como bitácoras) y la de Gestión de salas... pudiendo así integrarlas con éxito a la carpeta de las rutas de Laravel \cite{FusionAuth:Docs/Laravel}. La figura \ref{fig:RutasAURA}, muestra los endpoints de utiliza el proyecto.
...
Ahora bien, basandome en las plantillas y en la paleta de colores que me proporciono la Agencia, empece a desarrollar ahora si que el Frontend de la plataforma... utilizando tecnologías como blade y Tailwinds CSS. Empezemos por la figura \ref{fig:loginFrontend}, que funciona como la puerta de entrada al sistema AURA. Aquí es donde el usuario se debe loguear obligatoriamente para poder avanzar al dashboard principal. No utiliza contraseñas locales; todo el proceso de identidad y seguridad se delega a FusionAuth. Al colocar mis credenciales institucionales, el sistema reconoce automáticamente mi identidad y, lo más importante, mi rol.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/loginFrontend.png}
    \caption{Login oficial del proyecto por parte de la agencia.}
    \label{fig:loginFrontend}
\end{figure}

Ahora pasemos al Dashboard Principal.

Es el centro de control del día a día. Aquí es donde se visualizan los datos operativos en tiempo real a través de cuatro tarjetas rápidas (Visitantes Hoy, Visitas Activas, Llamadas Registradas y Salas Ocupadas). Si llega alguien a la recepción, la recepcionista no tiene que buscar en menús ocultos; justo al centro tiene la sección de Acciones Rápidas con botones grandes para registrar un nuevo visitante, escanear el rostro de un recurrente o apuntar una llamada.
Además, en la parte inferior hay una tabla dinámica que le recuerda exactamente quién sigue dentro del edificio y cuánto tiempo lleva ahí, permitiendo darles salida con un solo clic. La figura \ref{fig:dashboardFrontend}, muestra la interfaz del dashboard.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/dashboardFrontend.png}
    \caption{Dashboard de la plataforma del proyecto.}
    \label{fig:dashboardFrontend}
\end{figure}

El siguiente catalogo es el Módulo de Visitantes.

Este es el flujo crítico para eliminar las bitácoras de papel. En la columna izquierda se capturan los datos obligatorios (nombre, identificación y a qué funcionario busca). Lo interesante ocurre en la columna derecha: aquí se activa la cámara web para tomarle una fotografía en vivo. Al marcar la casilla de Embedding Facial, el sistema activa un algoritmo inteligente que convierte esa foto en un código matemático único. Así, al presionar "Registrar Entrada", la persona queda guardada para siempre en la base de datos. La figura \ref{fig:moduloVisitantes}, muestra la UI correspondiente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/moduloVisitantes.png}
    \caption{Catalogo para el registro de visitantes dentro de la plataforma.}
    \label{fig:moduloVisitantes}
\end{figure}

Pasemos al Módulo del Reconocimiento Facial.

 En lugar de volver a pedirle todos los datos a un ciudadano que va seguido, la recepcionista activa esta pantalla, la cual muestra la cámara en grande con una guía para centrar el rostro. En un par de segundos, el sistema busca ese rostro en la base de datos. Si hay coincidencia, aparece mágicamente un panel lateral con su nombre y foto de referencia.
Este módulo tiene una función vital de protección. Si el visitante fue bloqueado previamente por la dirección, saltará una alerta roja brillante con el motivo del bloqueo, permitiendo a la recepcionista tomar medidas de seguridad antes de permitir el ingreso. Si todo está en orden, un botón inicia la visita en un segundo. La figura \ref{fig:reconocimientoBiometrico}, muestra la interfaz para el reconocimiento facial del visitante.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/reconocimientoBiometrico.png}
    \caption{Catalogo para el registro biometrico del visitante.}
    \label{fig:reconocimientoBiometrico}
\end{figure}


Ahora toca hablar sobre el Módulo de Llamadas.

Este catalogo sirve para llevar la bitácora telefónica digital de la Agencia. En la parte de arriba se capturan rápido los datos (quién llama, de qué número y a qué funcionario o área se le transfiere). Al guardarla, se alimenta automáticamente la tabla de abajo, permitiendo tener un registro histórico transparente de qué recepcionista atendió cada llamada y cuáles fueron los asuntos más consultados del día. La figura \ref{fig:moduloLlamadas}, muestra la UI del catalogo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/moduloLlamadas.png}
    \caption{Catalogo de llamadas del proyecto.}
    \label{fig:moduloLlamadas}
\end{figure}

También se desarrollo el Frontend para el Módulo de Salas.

Administra los espacios de juntas sin que haya pleitos por los horarios. Es un calendario visual interactivo donde cada sala tiene su propio color. La recepcionista puede ver los bloques de horas ocupadas y, si da clic en un hueco vacío, se abre un formulario para agendar una nueva reunión. Lo mejor es que la base de datos tiene un candado inteligente: si intentas empalmar una junta con otra en la misma sala y hora, el sistema bloquea la acción y te avisa que el espacio ya está reservado. La figura \ref{fig:moduloSalas}, Muestra la interfaz correspondiente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/moduloSalas.png}
    \caption{Módulos de salas de la plataforma.}
    \label{fig:moduloSalas}
\end{figure}

Pasemos al modulo de Reportes Operativos e Históricos.

El auditor selecciona qué quiere investigar (Visitas, Llamadas, Salas o Eventos) a través de tarjetas visuales. Al elegir una, el panel de filtros cambia dinámicamente para pedirle las fechas exactas o los nombres que desea filtrar.
Para no descargar archivos a ciegas, el sistema genera una tabla interactiva en la pantalla con una muestra real de los datos encontrados en la base de datos (fecha de entrada, motivo, gafete, etc.). Si la información es la correcta, el usuario utiliza los botones de exportación para descargar el reporte en PDF Institucional listo para firmar o en Excel para hacer cruces de datos avanzados. La figura \ref{fig:moduloReportes}, muestra el frontend del catalogo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/moduloReportes.png}
    \caption{Módulos de reportes de la plataforma.}
    \label{fig:moduloReportes}
\end{figure}

Y por ultimo pasemos al Panel de Administración.

Aquí es donde se supervisa y configura todo el sistema a nivel macro. Se compone de tres áreas clave:

Dashboard de Métricas Avanzadas: Aquí se visualizan gráficas complejas cuya función es la toma de decisiones. Por ejemplo: la gráfica de Visitantes por mes ayuda a ver las temporadas de mayor afluencia; la de Uso de salas ayuda a saber si los espacios se están aprovechando; y la de Funcionarios más visitados revela qué áreas tienen mayor carga de atención ciudadana.

Gestión de Catálogos: Sirve para alimentar el sistema. Si entra un funcionario nuevo, si se crea un área nueva o si se compra una pantalla para una sala, aquí es donde el administrador da de alta esos datos para que aparezcan en los formularios de la recepción.

Configuración del Sistema y Auditoría: Permite ver la Bitácora de Eventos. Esta es una herramienta forense que registra absolutamente todo (quién inició sesión, a qué hora, qué registros se borraron o modificaron y desde qué dirección IP). Es crucial para auditorías de seguridad.

La figura \ref{fig:panelAdministracion}, muestra la interfaz correspondiente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figs/panelAdministracion.png}
    \caption{Panel de administración para los administradores de la plataforma.}
    \label{fig:panelAdministracion}
\end{figure}
...
La plataforma al estar utilizando la funcionalidad del reconocimiento facial, decidí implementar una librería llamada Face-API, que permite extraer las fotografías tomadas de los visitantes para así poder recolectar los datos de las fotos y guardarlas en la base de datos. Al estar trabajando con postgreSQL, tuve la la oportunidad de instalar una extensión llamada pgvector... una extensión exclusiva para el postgres que permite guardar embeddings faciales en las tablas, es decir, en vez de guardar imágenes pesadas, podemos guardar las fotos en un formato de números y vectores para así poder agilizar la búsqueda similar de fotografías.


\clearpage
\subsection{Ejecución de pruebas de rendimiento, seguridad y consistencia en entornos Docker}
Debido a que en el celular es imposible manejar comandos de docker por las limitantes que posee el kernel de Android, el proyecto se tuvo que migrar a otro dispositivo, específicamente a una laptop que me proporciono la agencia... para que de esta manera, pudiera manejar los contenedores docker sin ninguna limitante. Para la elaboración de las pruebas, se utilizo un archivo llamado docker-compose.yml para la orquestación de los distintos componentes de la plataforma. Este archivo fue desarrollado por la propia agencia y ya venia incluido en el starter kit. La gestión de los roles se realizaron con la ayuda de una librería de laravel llamada Spatie \cite{Spatie:documentacion}. Spatie es una librería que se puede integrar en proyectos de Laravel y que nos permite gestionar los privilegios y autenticación de los usuarios mediante los roles.


\clearpage
\subsection{Elaboración de documentación técnica, diccionarios de datos y manuales de usuario}
En esta etapa, comencé a desarrollar el documento de análisis del proyecto, un archivo que contendría todo el contenido relacionado al proyecto, desde su alcance, las necesidades que busca resolver, su marco legal, requerimientos, el publico al que esta dirigido el proyecto, hasta los requerimientos legales que debe respetar la agencia para no comprometer los datos confidenciales de los trabajadores de la agencia a la hora de estar utilizando la plataforma. Dentro de este archivo he redactado los requisitos funcionales y no funcionales. En cada Requisito funcional, he redactado una historia de usuario, junto con sus criterios de aceptación con la finalidad de reducir ambigüedades y explicar a detalle el comportamiento de los usuarios al estar ejecutando esos requisitos funcionales.

Algo que quiero destacar, es de que conocí a muchos empleados que me enseñaron como trabajan y a lo que se dedican. Entre ellos, conocí a una persona que se dedica al área de análisis de proyectos... tuve la fortuna de que ese compañero fuera amigo de mi madre, así que me estuvo enseñando muchas cosas, desde como redactar un buen documento de análisis con sus respectivos títulos, hasta el flujo de trabajo que siguen según el área en el que se encuentren desempeñando.

Después de terminar de redactar el documento de Análisis, inicie con el desarrollo del manual técnico del proyecto... el documento lo que busca es explicar los componentes que utiliza la plataforma barra proyecto, así como también su arquitectura, endpoints, stack tecnológico, modelo de datos, su infraestructura y despliegue, audiencia a la que esta dirigida la plataforma, etc. De esta manera, tanto los desarrolladores de la agencia, como los directivos, comprenderán como funciona por dentro el proyecto.

En del manual técnico, específicamente dentro de una sección llamada Modelos de datos, viene el diccionario de datos. En el diccionario de datos se explica la descripción de cada tabla, así como también sus atributos, sus tipos de datos, relaciones, restricciones y al final una breve descripción de cada campo... todo esta información esta encapsulada en tablas comparativas.

Al haber terminado de desarrollar el documento técnico, comencé con la redacción del manual de usuario... un documento que les ayudaría a los recepcionistas y administradores de la agencia a como utilizar el proyecto una vez finalizado y desplegado.

\clearpage
\section{Resultados}
Ya una vez abordado toda la metodología, es hora de comenzar a redactar la cereza del pastel... estamos hablando de los resultados de mi proyecto. Para que se pueda comprender del todo el funcionamiento del proyecto, tratare de explicar paso a paso el como funciona cada vista, mientras voy indexando algunas figuras para las representaciones visuales de las UI.

Primero empecemos por el login. El usuario es llevado a una vista en donde se le solicitara unas credenciales proporcionadas por la propia agencia. Esta vista se mantiene viva gracias a una plataforma que ellos desarrollaron llamada "AccesoTamaulipas" \cite{AIIDT:accesoTamaulipas}. Esta plataforma utiliza el servicio de FusionAuth para así poder validar las credenciales y una vez verificadas, reidericcionar al usuario a la pagina de inicio del proyecto. En la figura \ref{fig:loginPlataforma} se muestra la vista correspondiente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/loginAURA.jpeg}
    \caption{Login de la plataforma.}
    \label{fig:loginPlataforma}
\end{figure}

Una vez que el usuario se haya logueado en la plataforma, este sera reidereccionado a la vista del dashboard, en donde el usuario podrá visualizar en tiempo real todo lo que vaya pasando dentro de la agencia. En la figura \ref{fig:finalDashboard}, se puede visualizar la pagina del dashboard.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/FinalDahsboard.png}
    \caption{Dashboard del proyecto.}
    \label{fig:finalDashboard}
\end{figure}

Pasemos al catalogo de visitantes. Los usuarios serán capaces de registrar a los visitantes por medio del reconocimiento facial. Los usuarios deben de utilizar una cámara física para poder capturar el rostro de los visitantes. Una vez capturados sus rostros y almacenados en la base de datos, se procederá a registrar la visita del visitante. En la figura \ref{fig:finalVisitantes1}, se puede visualizar la vista descrita.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalVisitantes1.png}
    \caption{Registro de un visitante desde cero.}
    \label{fig:finalVisitantes1}
\end{figure}

Por su puesto, si el visitante suele visitar mucho la agencia, en vez de volver a solicitarle los datos, se usara la cámara directamente en vez de rellenar sus datos en los campos de texto. En la figura \ref{fig:finalVisitantes2}, se puede visualizar esta segunda vista.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalVisitantes2.png}
    \caption{Identificar a visitante recurrente.}
    \label{fig:finalVisitantes2}
\end{figure}

Ahora pasemos al catalogo de llamadas. Aquí los usuarios de la agencia podrán registrar todas las llamadas que van recibiendo los recepcionistas. En la figura \ref{fig:finalLlamadas}, se muestra a detalle el catalogo de llamadas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalLlamadas.png}
    \caption{Registro de llamadas.}
    \label{fig:finalLlamadas}
\end{figure}

Ahora pasemos al catalogo de salas, aquí los usuarios podrán reservar salas de juntas para los visitantes de la agencia. Cada sala tiene un color que los representan, así como también el equipamiento que llevan, como monitores, cuantas sillas hay, etc. En la figura \ref{fig:finalLlamadas}, se muestra tanto el calendario de las reservaciones, como los campos que deben de rellenar para reservar una sala.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalSalas.png}
    \caption{El catalogo de salas de reservaciones.}
    \label{fig:finalSalas}
\end{figure}

Ahora pasemos al catalogo de reportes. Aquí los usuarios tanto recepcionistas, como administradores, tendrán la oportunidad de generar reportes acerca de todo lo que ha estado pasando dentro de la agencia... como por ejemplo, cuantas salas se han reservado hoy ,cantidad de visitas activas el día de hoy, visitantes que han llegado a la agencia, etc. En la figura \ref{fig:Finalreportes}, se muestra a detalle la siguiente vista.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/Finalreportes.png}
    \caption{El catalogo de la generación de reportes del proyecto.}
    \label{fig:Finalreportes}
\end{figure}

Y por ultimo, pasemos a la vista de Administración. Esta vista solo podrá ser visualizada por los administradores y ellos tendrán el derecho de gestionar la existencia de algunas salas, así como también supervisar el estado actual de los funcionarios. En la figura \ref{fig:finalAdmin}, se puede visualizar esta vista.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalAdmin.png}
    \caption{El panel final de administración del proyecto.}
    \label{fig:finalAdmin}
\end{figure}

\clearpage
\section{Conclusiones}
Las estadías TSU tuvieron un impacto en mi desarrollo social, permitiéndome integrarme
a varios círculos sociales de manera amigable, así como también se moldeo mas la habilidad
del trabajo en equipo. Durante el periodo, se desempeño la habilidad de realizar levantamiento
de requerimientos, elaborar documentos de análisis, así como tambien la implementación de un sistema
rígido, la integridad de servicios externos de seguridad y la realización de pruebas en tiempo
real. Las estadías no solo me aportaron mas conocimiento o amistades, sino que se logro ver en
acción el flujo real de trabajo de un equipo de desarrolladores... así mismo, se vieron tecnologías
que no se habían visto antes de la universidad. Los proyectos de la Agencia de Innovación e
Inteligencia Digital de Tamaulipas demostraron un gran desafió e impacto para nosotros como
estudiantes, ya que solemos tener una percepción diferente del mundo laboral antes de ir al
batallón. También se puso a prueba la resolución de problemas a nivel empresarial, puesto que
como estudiante, se puso a marcha el manejo de esta habilidad con la ayuda de los empleados
