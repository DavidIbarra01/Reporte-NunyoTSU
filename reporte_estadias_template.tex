...
Ahora bien, basandome en las plantillas y en la paleta de colores que me proporciono la Agencia, empece a desarrollar ahora si que el Frontend de la plataforma... utilizando tecnologías como blade y Tailwinds CSS. Empezemos por la figura \ref{fig:loginFrontend}, que funciona como la puerta de entrada al sistema AURA. Aquí es donde el usuario se debe loguear obligatoriamente para poder avanzar al dashboard principal. No utiliza contraseñas locales; todo el proceso de identidad y seguridad se delega a FusionAuth. Al colocar mis credenciales institucionales, el sistema reconoce automáticamente mi identidad y, lo más importante, mi rol, redirigiéndome a la vista que me corresponde.
...
Tambien se desarrollo el Frontend para el Módulo de Salas.
...
\begin{figure}[H]
    ...
    \caption{Modulos de salas de la plataforma.}
...
\begin{figure}[H]
    ...
    \caption{Modulos de reportes de la plataforma.}
...
\begin{figure}[H]
    ...
    \caption{Panel de administracion para los administradores de la plataforma.}
...
La plataforma al estar utilizando la funcionalidad del reconocimiento facial, decidí implementar una librería llamada faceapi, que permite extraer las fotografías tomadas de los visitantes para así poder recolectar los datos de las fotos y guardarlas en la base de datos.


\clearpage
\subsection{Ejecución de pruebas de rendimiento, seguridad y consistencia en entornos Docker}
Para la elaboración de las pruebas, se utilizo un archivo llamado dockercompose.yml para la orquestación de los distintos componentes de la plataforma. Este archivo fue desarrollado por la propia agencia y ya venia incluido en el starter kit. La gestion de los roles se realizaron con la ayuda de una libreria de laravel llamada Spatie \cite{Spatie:documentacion}.


\clearpage
\subsection{Elaboración de documentación técnica, diccionarios de datos y manuales de usuario}
En esta etapa, comence a desarrollar el documento de analisis del proyecto, un archivo que contendria todo el contenido relacionado al proyecto, desde su alcance, las necesidades que busca resolver, su marco legal, requerimentos, el publico al que esta dirigido el proyecto, hasta los requerimientos legales que debe respetar la agencia para no comprometer los datos confidenciales de los trabajdores de la agencia a la hora de estar utilizando la plataforma. Dentro de este archivo he redactado los requisitos funcionales y no funcionales. En cada Requisito funcional, he redactado una historia de usuario, junto con sus criterios de aceptacion con la finalidad de reducir ambiguedades y explicar a detalle el comportamiento de los usuarios al estar ejecutando esos requisitos funcionales.

Algo que quiero destacar, es de que conoci a muchos empleados que me enseñaron como trabajan y a lo que se dedican. Entre ellos, conoci a una persona que se dedica al area de analisis de proyectos... tuve la fortuna de que ese compañero fuera amigo de mi madre, asi que me estuvo enseñando muchas cosas, desde como redactar un buen documento de analisis con sus respectivos titulos, hasta el flujo de trabajo que siguen segun el area en el que se encuentren desempeñando.

Despues de terminar de redactar el documento de Analisis, inicie con el desarrollo del manual tecnico del proyecto... el documento lo que busca es explicar los componentes que utiliza la plataforma barra proyecto, asi como tambien su arquitectura, endpoints, stack tecnologico, modelo de datos, su infraestructura y despliegue, audiencia a la que esta dirigida la plataforma, etc. De esta manera, tanto los desarrolladores de la agencia, como los directivos, comprenderan como funciona por dentro el proyecto.

En del manual tecnico, especificamente dentro de una seccion llamada Modelos de datos, viene el diccionario de datos. En el diccionario de datos se explica la descripcion de cada tabla, asi como tambien los atributos que utilia, sus tipos de datos, relaciones, restricciones y al final una breve descripcion de cada campo... todo esta informacion esta encapsulada en tablas comparativas.

Al haber terminado de desarrollar el documento tecnico, comence con la redaccion del manual de usuario... un documento que les ayudaria a los recepcionistas y administradores de la agencia a como utilizar el proyecto una vez finalizado y desplegado.

\clearpage
\section{Resultados}
Ya una vez abordado toda la metodología, es hora de comenzar a redactar la cereza del pastel... estamos hablando de los resultados de mi proyecto. Para que se pueda comprender del todo el funcionamiento del proyectp, tratare de explicar paso a paso el como funciona cada vista, mientras voy indexando algunas figuras para las representaciones visuales de las UI.

Primero empezemos por el login. El usuario es llevado a una vista en donde se le solicitara unas credenciales proporcionadas por la propia agencia. Esta vista se mantiene viva gracias a una plataforma que ellos desarrollaron llamada "AccesoTamaulipas" \cite{AIIDT:accesoTamaulipas}. Esta plataforma utiliza el servicio de FusionAuth para asi poder validar las credenciales y una vez verificadas, reidericcionar al usuario a la pagina de inicio del proyecto. En la figura \ref{fig:loginPlataforma} se muestra la vista correspondiente.
...
Una vez que el usuario se haya logueado en la plataforma, este sera reidereccionado a la vista del dashboard, en donde el usuario podra visualizar en tiempo real todo lo que vaya pasando dentro de la agencia. En la figura \ref{fig:finalDashboard}, se puede visualizar la pagina del dashboard.
...
Pasemos al catalogo de visitantes. Los usuarios serán capaces de registrar a los visitantes por medio del reconocimiento facial. Los usuarios deben de utilizar una cámara física para poder capturar el rostro de los visitantes. Una vez capturados sus rostros y almacenados en la base de datos, se procedera a registrar la visita del visitante. En la figura \ref{fig:finalVisitantes1}, se puede visualizar la vista descrita.
...
Por su puesto, si el visitante suele visitar mucho la agencia, en vez de volver a solicitarle los datos, se usara la camara directamente en vez de rellenar sus datos en los campos de texto. En la figura \ref{fig:finalVisitantes2}, se puede visualizar esta segunda vista.
...
Ahora pasemos al catalogo de llamadas. Aqui los usuarios de la agencia podran registrar todas las llamadas que van recibiendo los recepcionistas. En la figura \ref{fig:finalLlamadas}, se muestra a detalle el catalogo de llamadas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/finalLlamadas.png}
    \caption{Registro de llamdas.}
    \label{fig:finalLlamadas}
\end{figure}

Ahora pasemos al catalogo de salas, aqui los usuarios podran reservar salas de juntas para los visitantes de la agencia. Cada sala tiene un color que los representan, asi como tambien el equipamiento que llevan, como monitores, cuantas sillas hay, etc. En la figura \ref{fig:finalLlamadas}, se muestra tanto el calendario de las reservaciones, como los campos que deben de rellenar para reservar una sala.
...
Las estadías TSU tuvieron un impacto en mi desarrollo social, permitiéndome integrarme a varios círculos sociales de manera amigable, así como también se moldeo mas la habilidad del trabajo en equipo. Durante el periodo, se desempeño la habilidad de realizar levantamiento de requerimientos, elaborar documentos de análisis, así como la implementación de un sistema rígido, la integridad de servicios externos de seguridad y la realización de pruebas en tiempo real.
